...
void SceneGame::Initialize()
{
	...
	{
		Light* light = new Light(LightType::Point);
		light->SetPosition(DirectX::XMFLOAT3(-10.698f, 4, -3.111f));
		light->SetColor(DirectX::XMFLOAT4(1, 1, 1, 1));
		light->SetRange(5.0f);
		LightManager::Instance().Register(light);
	}
	...
	turret1->SetPosition(DirectX::XMFLOAT3(-3.0f, 0.2f, 0.1f));
	EnemyManager::Instance().Register(turret1);

	Turret* turret2 = new Turret();
	turret2->SetPosition(DirectX::XMFLOAT3(6.0f, 0.2f, 0.1f));
	EnemyManager::Instance().Register(turret2);

	//スカイボックス
	skybox = new SkyBox(std::make_shared<Texture>("Data/SkyBox/beautiful_sky2.jpg"));
...
void SceneGame::Update(float elapsedTime)
{
	...
	effectController.Update(elapsedTime);
	// ステージ更新処理
