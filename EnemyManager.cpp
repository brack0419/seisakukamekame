...
{
	return rand() % ENEMY_TYPE; // 0,1,2 のどれか
}

void EnemyManager::Spawn(float elapsedTime)
{
	if (spwan_time >= 0.1f)
	{
		if (ENEMY_MAX >= enemy_max)
		{
			DirectX::XMFLOAT3 position =
			{
				((static_cast<float>(rand() * 2) / RAND_MAX) - 1.0f) * 50.0f,
				0.0f,
				((static_cast<float>(rand() * 2) / RAND_MAX) - 1.0f) * 50.0f,
			};
			Enemy* enemy = nullptr;
